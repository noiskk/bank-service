...
| | 팀 프로젝트 | 개인 고도화 |
|---|---|---|
| 출금 | 3단계 안전 출금 + 비관적 락 | (유지) |
| 취소 | 없음 | **출금 취소 API 신규 (멱등 보장)** |
| 원거래 추적 | `referenceId` 컬럼만 있고 사용 안 함 | **카드사 거래번호를 저장해 취소·대사에서 원거래를 특정** |
| 거절 응답 | HTTP 422 | **HTTP 200 + 응답코드** (아래 참고) |
| 예외 | 문자열 메시지에 `"부족"` 포함 여부로 분기 | 타입 있는 예외 + 전역 핸들러 |
| 화면 | 없음 | 계좌 관제 |

### 거절을 200으로 바꾼 이유 — 실제로 있던 버그

잔액부족·계좌없음 같은 **비즈니스 거절을 HTTP 422로 응답**하고 있었다. 그런데 이 API를 호출하는 건 카드사의 OpenFeign 클라이언트뿐이고, **Feign은 비2xx를 받으면 정상 객체가 아니라 예외(`FeignException`)를 던진다.**

그 결과 잔액부족처럼 매일 일어나는 정상 거절이 카드사 입장에서는 전부 **"은행 호출 실패, 출금 여부 불확실"**로 오분류되고 있었다. 카드사는 이를 보상·대사 대상으로 처리하려 했을 것이다.
...
또 컨트롤러가 `throw new IllegalStateException("잔액 부족")`으로 예외를 만들고 몇 줄 아래 `catch`에서 `e.getMessage().contains("부족")`으로 **문자열을 다시 검사하는 자기 완결 왕복 구조**였다. 타입 있는 예외(`InsufficientBalanceException`)를 던지고 전역 핸들러에 맡겨 이 왕복을 제거했다.
